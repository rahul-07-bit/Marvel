    ...
    <>
      ...
      >
        <img 
          ...
          className="w-[95vw] md:w-[75vw] lg:w-[65vw] max-w-[1300px] object-contain mix-blend-screen"
        />
      </div>
      ...
      <div className={`transition-opacity duration-[2000ms] ${introPhase === 'complete' ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
          {/* TopNavBar */}
          <header className="bg-black/40 backdrop-blur-md text-white docked full-width top-0 border-b border-white/5 flex items-center w-full px-12 h-[80px] max-w-none fixed z-[80] justify-between pointer-events-none transition-all duration-500">
            {/* Left Section: Logo */}
            <div className="flex items-center pointer-events-auto cursor-pointer group" onClick={() => setCurrentTab('home')}>
              <div className="bg-[#EC1D24] px-2 pt-1.5 pb-1 flex items-center justify-center group-hover:brightness-125 transition-all">
                <span className="font-marvel text-5xl text-white tracking-[-0.02em] leading-none uppercase mix-blend-screen scale-y-110">MARVEL</span>
              </div>
            </div>
            ...
            <nav className="hidden lg:flex items-center gap-10 pointer-events-auto absolute left-1/2 -translate-x-1/2">
                ...
                <button 
                  ...
                  className={`font-label text-[0.7rem] tracking-[0.4em] uppercase font-bold transition-all duration-500 hover:text-[#EC1D24] ${currentTab === 'timeline' ? 'text-[#EC1D24]' : 'text-gray-400'}`}
                >
                  TIMELINE
                </button>
                <span className="text-white/10 font-light select-none">|</span>
                <button 
                  ...
                  className={`font-label text-[0.7rem] tracking-[0.4em] uppercase font-bold transition-all duration-500 hover:text-[#EC1D24] ${currentTab === 'story' ? 'text-[#EC1D24]' : 'text-gray-400'}`}
                >
                  STORY
                </button>
                <span className="text-white/10 font-light select-none">|</span>
                <button 
                  onClick={() => setCurrentTab('villain')}
                  className={`font-label text-[0.7rem] tracking-[0.4em] uppercase font-bold transition-all duration-500 hover:text-[#EC1D24] ${currentTab === 'villain' ? 'text-[#EC1D24]' : 'text-gray-400'}`}
                >
                  VILLAIN
                </button>
            </nav>
            ...
            <div className="flex items-center gap-10 pointer-events-auto">
              <button 
                className="relative p-2 rounded-full flex items-center justify-center transition-all duration-500 hover:scale-110 active:scale-95 group pointer-events-auto"
                onClick={handleNotificationClick}
              >
                <span className="material-symbols-outlined text-[2rem] text-gray-300 group-hover:text-white transition-all duration-500 group-hover:drop-shadow-[0_0_12px_rgba(255,255,255,0.3)]" style={{ fontVariationSettings: "'FILL' 0, 'wght' 200, 'GRAD' 0, 'opsz' 24" }}>notifications</span>
                
                {hasUnread && (
                  <div className="absolute top-[0.4rem] right-[0.4rem] w-[7px] h-[7px] bg-[#ed1d24] rounded-full shadow-[0_0_15px_rgba(237,29,36,0.9)] animate-[badgePop_0.4s_cubic-bezier(0.175,0.885,0.32,1.275)]"></div>
                )}
              </button>

              <button 
                className="hover:opacity-70 transition-opacity duration-600" 
                onClick={() => setIsMenuOpen(true)}
              >
                <span className="material-symbols-outlined font-light text-[2rem]" style={{ fontVariationSettings: "'FILL' 0, 'wght' 200, 'GRAD' 0, 'opsz' 24" }}>menu</span>
              </button>
            </div>
          </header>
          ...
          <div 
            className={`fixed top-32 right-12 z-[120] w-[380px] bg-[#0a0a0a]/80 backdrop-blur-2xl rounded-2xl border border-white/5 shadow-[0_20px_40px_-15px_rgba(0,0,0,0.7)] px-6 py-5 flex items-start gap-4 transition-all duration-500 ease-out transform ${
